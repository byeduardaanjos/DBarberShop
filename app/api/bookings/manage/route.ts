...
export async function PATCH(request:NextRequest){
  ...
  if(body.action==="reschedule"&&(!/^\d{4}-\d{2}-\d{2}$/.test(body.date??"")||!times.has(body.time??"")||!isBookingTimeAllowed(body.date??"",body.time??"")))return NextResponse.json({error:"Escolha uma nova data e horário."},{status:400});
  const supabase=config(); if(!supabase)return NextResponse.json({error:"Banco não configurado."},{status:503});
