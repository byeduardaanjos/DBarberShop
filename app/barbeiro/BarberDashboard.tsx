...
import {
  ...
  LoaderCircle, LockKeyhole, LogOut, MessageCircle, Phone, RefreshCw, Save, Scissors, Search,
  UserRound, Users, UserX, XCircle,
} from "lucide-react";
import AvailabilityManager from "./AvailabilityManager";
import BarberBookingModal, { NewBookingButton } from "./BarberBookingModal";
import Image from "next/image";

type BookingStatus = "confirmed" | "completed" | "cancelled" | "no_show";
type Booking = { id:string; customer_name:string; customer_phone:string; booking_date:string; booking_time:string; status:BookingStatus; booking_source?:"site"|"barber"; selected_services?:string[]; total_price_cents?:number; services:{name:string;duration_minutes:number}|null };
...
const formatBookingDate = (value:string) => new Date(`${value}T12:00:00`).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric"});
const whatsappNumber = (phone:string) => { const digits=phone.replace(/\D/g,""); return digits.startsWith("55")?digits:`55${digits}`; };

export default function BarberDashboard(){
  const [auth,setAuth]=useState<"loading"|"guest"|"barber">("loading");
  const [view,setView]=useState<View>("agenda");
  const [email,setEmail]=useState(""); const [password,setPassword]=useState("");
  const [date,setDate]=useState(today); const [bookings,setBookings]=useState<Booking[]>([]); const [upcomingBookings,setUpcomingBookings]=useState<Booking[]>([]); const [customers,setCustomers]=useState<Customer[]>([]);
  ...
  const [cancelBooking,setCancelBooking]=useState<Booking|null>(null); const [cancelReason,setCancelReason]=useState(""); const [cancelling,setCancelling]=useState(false);
  ...
  const filteredBookings=useMemo(()=>{const q=agendaSearch.trim().toLocaleLowerCase("pt-BR");return bookings.filter(item=>(statusFilter==="all"||item.status===statusFilter)&&(!q||item.customer_name.toLocaleLowerCase("pt-BR").includes(q)||item.customer_phone.includes(q)||item.services?.name.toLocaleLowerCase("pt-BR").includes(q)||item.selected_services?.some(service=>service.toLocaleLowerCase("pt-BR").includes(q))));},[agendaSearch,bookings,statusFilter]);
  ...
  async function updateStatus(id:string,status:BookingStatus){setMessage("");const response=await fetch("/api/barbeiro/agendamentos",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id,status})});const data=await response.json();if(!response.ok){setMessage(data.error??"Não foi possível atualizar.");return false;}await Promise.all([loadBookings(date,true),loadUpcoming()]);if(customers.length)await loadCustomers(true);return true;}
  function requestCancellation(booking:Booking){setCancelBooking(booking);setCancelReason("");setMessage("");}
  async function confirmCancellation(){
    if(!cancelBooking||cancelReason.trim().length<3)return;
    setCancelling(true);
    const booking=cancelBooking;
    const success=await updateStatus(booking.id,"cancelled");
    setCancelling(false);
    if(!success)return;
    const service=booking.selected_services?.join(" + ")??booking.services?.name??"Atendimento";
    const text=[
      `Olá, ${booking.customer_name.split(" ")[0]}. Precisamos cancelar seu atendimento na D.BarberShop.`,
      "",
      `Serviço: ${service}`,
      `Data: ${formatBookingDate(booking.booking_date)}`,
      `Horário: ${booking.booking_time.slice(0,5)}`,
      `Motivo: ${cancelReason.trim()}`,
      "",
      "Pedimos desculpas pelo imprevisto. Você pode escolher um novo horário pelo nosso site:",
      `${window.location.origin}/?agendar=1`,
    ].join("\n");
    setCancelBooking(null);setCancelReason("");
    window.location.href=`https://wa.me/${whatsappNumber(booking.customer_phone)}?text=${encodeURIComponent(text)}`;
  }
  ...
  return <main className="barber-dashboard">
    ...
    <BarberBookingModal open={bookingModalOpen} initialDate={date} onClose={()=>setBookingModalOpen(false)} onCreated={bookingCreated}/>
    {cancelBooking&&<div className="barber-modal-backdrop" onMouseDown={event=>event.target===event.currentTarget&&!cancelling&&setCancelBooking(null)}><section className="barber-cancel-modal" role="dialog" aria-modal="true" aria-labelledby="cancel-booking-title"><button className="barber-modal-close" disabled={cancelling} onClick={()=>setCancelBooking(null)} aria-label="Fechar"><XCircle/></button><p className="barber-kicker">CANCELAMENTO · AVISO AO CLIENTE</p><h2 id="cancel-booking-title">Cancelar atendimento?</h2><p className="barber-modal-intro">O horário de <strong>{cancelBooking.customer_name}</strong> será liberado e, em seguida, o WhatsApp abrirá com o aviso pronto.</p><div className="barber-cancel-summary"><span><CalendarDays/> {formatBookingDate(cancelBooking.booking_date)}</span><span><Clock3/> {cancelBooking.booking_time.slice(0,5)}</span></div><label>Motivo do cancelamento<textarea autoFocus maxLength={180} value={cancelReason} onChange={event=>setCancelReason(event.target.value)} placeholder="Ex.: Tivemos um imprevisto e não conseguiremos realizar o atendimento neste horário."/></label>{message&&<p className="barber-modal-message" role="alert">{message}</p>}<div className="barber-cancel-actions"><button type="button" disabled={cancelling} onClick={()=>setCancelBooking(null)}>Manter agendamento</button><button type="button" disabled={cancelling||cancelReason.trim().length<3} onClick={confirmCancellation}>{cancelling?<LoaderCircle className="spin"/>:<MessageCircle/>}{cancelling?"Cancelando…":"Cancelar e avisar"}</button></div></section></div>}
  </main>;

  function UpcomingPanel(){if(upcomingBookings.length===0)return null;return <section className="barber-upcoming"><div className="barber-upcoming-head"><div><p className="barber-kicker">PRÓXIMOS AGENDAMENTOS</p><h2>O que vem pela frente.</h2></div><span>{upcomingBookings.length} próximos</span></div><div className="barber-upcoming-list">{upcomingBookings.map(item=><button type="button" key={item.id} className={item.booking_date===date?"active":""} onClick={()=>setDate(item.booking_date)}><span className="upcoming-date"><strong>{new Date(`${item.booking_date}T12:00:00`).toLocaleDateString("pt-BR",{day:"2-digit"})}</strong><small>{new Date(`${item.booking_date}T12:00:00`).toLocaleDateString("pt-BR",{month:"short"}).replace(".","")}</small></span><span className="upcoming-client"><strong>{item.customer_name}</strong><small>{item.services?.name??"Serviço"} · {item.booking_time.slice(0,5)}</small></span><ChevronRight size={16}/></button>)}</div></section>}

  function AgendaView(){return <><section className="barber-heading"><div><p className="barber-kicker">VISÃO DO DIA</p><h1>Agenda de trabalho.</h1><p>Acompanhe, filtre e atualize cada atendimento.</p><NewBookingButton onClick={()=>setBookingModalOpen(true)}/></div><span className="barber-day-number">{new Date(`${date}T12:00:00`).toLocaleDateString("pt-BR",{day:"2-digit"})}<small>{new Date(`${date}T12:00:00`).toLocaleDateString("pt-BR",{month:"short"}).replace(".","")}</small></span></section><section className="barber-stats" aria-label="Resumo do dia"><article><span><small>01</small>Horários</span><strong>{summary.total.toString().padStart(2,"0")}</strong></article><article><span><small>02</small>Confirmados</span><strong>{summary.confirmed.toString().padStart(2,"0")}</strong></article><article><span><small>03</small>Concluídos</span><strong>{summary.completed.toString().padStart(2,"0")}</strong></article><article className="missed"><span><small>04</small>Faltaram</span><strong>{summary.noShow.toString().padStart(2,"0")}</strong></article></section><section className="barber-agenda"><div className="barber-agenda-title"><div><p className="barber-kicker">ROTEIRO DE ATENDIMENTOS</p><h2>{new Date(`${date}T12:00:00`).toLocaleDateString("pt-BR",{weekday:"long",day:"2-digit",month:"long"})}</h2></div><button onClick={()=>loadBookings(date)} aria-label="Atualizar agenda"><RefreshCw className={loading?"spin":""} size={18}/> Atualizar</button></div><div className="barber-filters"><label><Search size={16}/><input aria-label="Buscar na agenda" placeholder="Buscar nome, telefone ou serviço" value={agendaSearch} onChange={e=>setAgendaSearch(e.target.value)}/></label><div>{(["all","confirmed","completed","no_show","cancelled"] as StatusFilter[]).map(filter=><button type="button" key={filter} className={statusFilter===filter?"active":""} onClick={()=>setStatusFilter(filter)}>{filter==="all"?"Todos":statusLabel[filter]}</button>)}</div></div>{message&&<p className="barber-error" role="alert">{message}</p>}{loading?<div className="barber-empty"><LoaderCircle className="spin"/><p>Carregando horários...</p></div>:filteredBookings.length===0?<div className="barber-empty"><CalendarDays/><h3>Nenhum atendimento encontrado.</h3><p>Altere os filtros ou escolha outra data.</p></div>:<div className="barber-booking-list">{filteredBookings.map(booking=><article className={`barber-booking ${booking.status}`} key={booking.id}><div className="barber-time"><span>HORÁRIO</span><strong>{booking.booking_time.slice(0,5)}</strong></div><div className="barber-booking-info"><div><span className={`barber-status ${booking.status}`}>{statusLabel[booking.status]}</span><h3>{booking.customer_name}</h3></div><ul><li><Scissors size={16}/>{booking.selected_services?.join(" + ") ?? booking.services?.name ?? "Serviço"}</li><li><strong>{((booking.total_price_cents ?? 0) / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</strong></li><li><Clock3 size={16}/>{booking.services?.duration_minutes??0} minutos</li><li><Phone size={16}/><a href={`tel:${booking.customer_phone.replace(/\D/g,"")}`}>{booking.customer_phone}</a></li></ul></div><div className="barber-actions">{booking.status!=="completed"&&<button className="complete" onClick={()=>updateStatus(booking.id,"completed")}><CheckCircle2 size={16}/> Concluir</button>}{booking.status!=="no_show"&&<button className="no-show" onClick={()=>updateStatus(booking.id,"no_show")}><UserX size={16}/> Não veio</button>}{booking.status!=="cancelled"&&<button onClick={()=>requestCancellation(booking)}><XCircle size={16}/> Cancelar</button>}{booking.status!=="confirmed"&&<button onClick={()=>updateStatus(booking.id,"confirmed")}><UserRound size={16}/> Reabrir</button>}</div></article>)}</div>}</section></>}
